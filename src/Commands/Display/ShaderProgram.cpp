...
namespace SPMEditor 
{
    ...
    ShaderProgram::ShaderProgram(const char* programName, std::initializer_list<display_shader_t> shaders) :
        ...
        m_UseRenderContext(true)
    {
        ...
        for (const display_shader_t& shader : shaders)
        {
            LogTrace("\tLinking Shader ID %u", shader.shader_index);
            glAttachShader(m_ShaderProgramId, shader.shader_index);
            ...
            while (error)
            {
                switch (error)
                {
                    case GL_INVALID_VALUE:
                        LogError("Shader Program %u failed to attach shader %u. Invalid value. Program or shader not generated by opengl.", m_ShaderProgramId, shader.shader_index);
                        break;
                    case GL_INVALID_OPERATION:
                        LogError("Shader Program %u failed to attach shader %u. Invalid operation. \n\tProgram / shader not protram / shader object.\n\tShader may already be attached.", m_ShaderProgramId, shader.shader_index);
                        break;
                }
