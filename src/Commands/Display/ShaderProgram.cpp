...
namespace SPMEditor 
{
        ...
        m_UseRenderContext(true)
    {
        ...
        LogTrace("Creating shader program {} with ID: {}", programName, m_ShaderProgramId);
        for (const Shader& shader : shaders)
        {
            LogTrace("\tLinking Shader ID {}", shader.m_ShaderIndex);
            ...
            while (error)
            {
                switch (error)
                {
                    case GL_INVALID_VALUE:
                        LogError("Shader Program {} failed to attach shader {}. Invalid value. Program or shader not generated by opengl.", m_ShaderProgramId, shader.m_ShaderIndex);
                        break;
                    case GL_INVALID_OPERATION:
                        LogError("Shader Program {} failed to attach shader {}. Invalid operation. \n\tProgram / shader not protram / shader object.\n\tShader may already be attached.", m_ShaderProgramId, shader.m_ShaderIndex);
                        break;
                }
        ...
        while (error)
        {
            ...
            {
                case GL_INVALID_VALUE:
                    LogError("Shader Program {} failed to link. Invalid value. Program not generated by opengl.", m_ShaderProgramId);
                    break;
                case GL_INVALID_OPERATION:
                    LogError("Shader Program {} failed to link. Invalid operation. \n\tProgram not a program object or  program is the currently active program object and transform feedback mode is active.", m_ShaderProgramId);
                    break;
            }
        ...
        if (!success)
        {
            ...
            LogError("Failed to link shader '{0}': {1} \n\tError Length: {2}", programName, (char*)infoLog, logSize);
        }
        else
        {
            LogTrace("\tProgram {} ({}) Linked successfully", programName, m_ShaderProgramId);
        }
        ...
        for (int i = 0; i < uniformCount; i++)
        {
            ...
            LogTrace("\tFound uniform '{}' with type {}", name, type);
            m_UniformLocations.emplace(std::make_pair(name, i));
        }

        for (const auto& thing : m_UniformLocations)
        {
            LogTrace("\tUniform {}: ({}, {})", m_UniformLocations.size(), thing.first, thing.second);
        }
    }
