...
// Private functions 
// =========================================
void GetBlockSize(TPLImageFormat format, int& width, int& height);
std::vector<Color> ReadBlock(const u8* data, ImageHeader header, int xPos, int yPos);
std::vector<Color> ReadImage(const u8* data, ImageHeader imageHeader);
std::vector<Color> ReadCMPRBlock(const u8* data);
std::vector<Color> ReadRGBA32Block(const u8* data);

TPL::Header TPL::Header::SwapBytes() {
    TPL::Header header = *this;
    byte_swap_array_stride_4(&header, 3);
    return header;
}

TPL::ImageOffset TPL::ImageOffset::SwapBytes() {
    ImageOffset offset = *this;
    byte_swap_array_stride_4(&offset, 2);
    return offset;
}

TPL::PaletteHeader TPL::PaletteHeader::SwapBytes() {
    PaletteHeader header = *this;
    header.entryCount = byte_swap_u16(header.entryCount);
        ...
        for (int x = 0; x < blockWidth; x++) {
            ...
            switch (header.format)
            {
                ...
                case TPLImageFormat::RGB565:
                    {
                        u16 val = byte_swap_u16(*(u16*)(data + i));
                        u8 r = (val >> 11) * 0x8;
                        u8 g = (val >> 5) * 0x4;
                        u8 b = (val >> 0) * 0x8;
...
std::vector<Color> TPL::ReadCMPRBlock(const u8* data)
{
    ...
    for (int subY = 0, dataOffset = 0; subY < 2; subY++) {
        for (int subX = 0; subX < 2; subX++, dataOffset += 8) {
            // Read the pixels
            u16 aVal = byte_swap_u16(*(u16*)(data + dataOffset));
            u16 bVal = byte_swap_u16(*(u16*)(data + dataOffset + 2));
            ...
            u32 pixelData = byte_swap_u32(*(u32*)(data + dataOffset + 4));
...
void TPL::Write(const std::string& path) {
    ...
    TPL::Header header = {
        .magic = byte_swap_int(0x0020AF30),
        .numImages = byte_swap_int((int)images.size()),
        .imageTableOffset = byte_swap_int(0xC),
    };
    ...
    for (size_t i = 0; i < images.size(); i++) {
        TPL::ImageOffset imageOffset = {
            .headerOffset = byte_swap_int((int)(imageTableStart + i * sizeof(TPL::ImageHeader))),
    ...
    for (size_t i = 0; i < images.size(); i++) {
        ...
        TPL::ImageHeader header = { 
            .height = byte_swap_u16(baseHeader.height),
            .width = byte_swap_u16(baseHeader.width),
            .format = (TPLImageFormat)byte_swap_u32((u32)baseHeader.format),
            .wrapS = (ImageWrapMode)byte_swap_u32((u32)baseHeader.wrapS), // TODO: Add user configured wrap mode
            .wrapT = (ImageWrapMode)byte_swap_u32((u32)baseHeader.wrapT), // TODO: Add user configured wrap mode
            .minFilter = byte_swap_u32(baseHeader.minFilter),
            .magFilter = byte_swap_u32(baseHeader.magFilter),
            .LODBias = byte_swap_float(baseHeader.LODBias),
        ...
        header.imageDataAddress = byte_swap_int(imageDataOffset);
